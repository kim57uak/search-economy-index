...
def register_ticker_tools(mcp):
    ...
    @mcp.tool(
        description="Search multiple Korean domestic stock tickers at once. Provide list like ['삼성전자', '카카오', 'SK하이닉스']"
    )
    def search_multiple_domestic_tickers(queries: list) -> Dict[str, Any]:
        try:
            logger.info(f"복수 국내 티커 검색: {queries}")
            result = service_manager.search_multiple_domestic_tickers(queries)
            return {"queries": queries, "results": result}
        except Exception as e:
            logger.error(f"복수 국내 티커 검색 실패: {e}")
            return {"error": str(e)}

    @mcp.tool(
        description="Search multiple overseas stock tickers at once. Provide list like ['Apple', 'Tesla', 'Microsoft']"
    )
    def search_multiple_overseas_tickers(queries: list) -> Dict[str, Any]:
        try:
            logger.info(f"복수 해외 티커 검색: {queries}")
            result = service_manager.search_multiple_overseas_tickers(queries)
            return {"queries": queries, "results": result}
        except Exception as e:
            logger.error(f"복수 해외 티커 검색 실패: {e}")
            return {"error": str(e)}

    @mcp.tool(
        description="Search multiple cryptocurrency tickers at once. Provide list like ['Bitcoin', 'Ethereum', 'Cardano']"
    )
    def search_multiple_crypto_tickers(queries: list) -> Dict[str, Any]:
        try:
            logger.info(f"복수 암호화폐 티커 검색: {queries}")
            result = service_manager.search_multiple_crypto_tickers(queries)
            return {"queries": queries, "results": result}
        except Exception as e:
            logger.error(f"복수 암호화폐 티커 검색 실패: {e}")
            return {"error": str(e)}
